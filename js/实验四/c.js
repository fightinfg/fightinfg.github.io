...
window.onload = function initCube() {
	...
	dlenthLoc = gl.getUniformLocation(program, "dlenth");
	gl.uniform3fv(dlenthLoc, dlenth);

    document.getElementById( "dxcon" ).onchange = function( event ){
		bxis=ax;
		bxis2=event.target.value;
    }
	document.getElementById( "dycon" ).onchange = function( event ){
		bxis=ay;
		bxis2=event.target.value;
	}
	document.getElementById( "dzcon" ).onchange = function( event ){
		bxis=az;
		bxis2=event.target.value;
	}
     render();
